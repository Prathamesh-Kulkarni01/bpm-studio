...
import { propertyRendererRegistry } from '@/lib/propertyRendererRegistry';
...
useEffect(() => {
  ...
  // 1. Grab the businessObject for this element…
  let bo = element.businessObject;
  if (!bo) return;

  // 2. If this is a Participant, follow processRef to the real process BO
  if (bo.$type === 'bpmn:Participant' && bo.processRef) {
    bo = bo.processRef;
  }

  console.log('Using businessObject:', bo);

  // 3. Helper that tries prefixes + direct props + $attrs
  const getAttributeValue = (propName: string): any => {
    const possiblePrefixes = ['', 'camunda:', 'bioc:', 'color:'];

    for (const prefix of possiblePrefixes) {
      const key = `${prefix}${propName}`;

      // direct prop (e.g. bo.isExecutable, bo.name, bo.id)
      if (bo[key] !== undefined) {
        return bo[key];
      }

      // prefixed XML attrs (e.g. bo.$attrs['camunda:displayStatus'])
      if (bo.$attrs?.[key] !== undefined) {
        return bo.$attrs[key];
      }
    }
  ...
  // 4. Build initialValues for *this* BO (process or other)
  ...
  schema.properties?.forEach((prop) => {
    ...
    if (prop.name.includes('.')) {
      ...
      // generic prefix/direct/attrs lookup
      value = getAttributeValue(prop.name);
    }

    initialValues[prop.name] =
      value !== undefined ? value : prop.defaultValue;

    console.log(`→ ${prop.name}:`, initialValues[prop.name]);
  });

  setValues(initialValues);
  setLoading(false);

  // 5. Tabs logic unchanged
  if (schema.tabs?.length) {
    const firstVisible = schema.tabs.find(tab =>
      !tab.visibility ||
      evaluateCondition(tab.visibility.condition, element, initialValues)
    );
    setActiveTab(
      firstVisible?.id
      ?? schema.defaultTab
...
useEffect(() => {
  async function loadOptions() {
    ...
    for (const prop of propsWithOptions) {
      try {
        // Fetch options for standard properties
        let options: PropertyOption[] = [];
        if (prop.name.startsWith('camunda:') || prop.name.startsWith('bioc:') || prop.name.startsWith('color:')) {
          // For custom properties like camunda, bioc, color, etc.
          const customOptions = await fetchCustomPropertyOptions(prop, element, values);
          options = customOptions || [];
        } else {
          // For regular properties
          options = await fetchPropertyOptions(prop, element, values);
        }
...
const handleValueChange = (propName: string, value: any) => {
  ...
  if (element && modeler) {
    try {
      ...
      // Handle custom properties (e.g., camunda, bioc, etc.)
      if (propName.startsWith('camunda:') || propName.startsWith('bioc:') || propName.startsWith('color:')) {
        // For custom properties, handle accordingly
        const updateObj: any = { [propName]: value };
        modeling.updateProperties(element, updateObj);
      } else {
        // For regular properties, handle the nested structure
  ...
  const renderPropertyInput = (prop: PropertyDefinition) => {
    ...
    const options = prop.options 
      ...
      : [];
    console.log(prop.renderer)
    // Check if a custom renderer is specified
    if (prop.renderer && propertyRendererRegistry.has(prop.renderer)) {
      const CustomRenderer = propertyRendererRegistry.get(prop.renderer)!;
      return (
        <CustomRenderer
          property={prop}
          value={value}
          onChange={(newValue) => handleValueChange(prop.name, newValue)}
          element={element}
          values={values}
        />
      );
        ...
        case 'text':
          return (
            <Input
              ...
              placeholder={typeof prop.placeholder === 'function' ? prop.placeholder(element, values) : prop.placeholder as string}
          ...
          return (
            ...
            >
              <SelectTrigger className="w-full">
                <SelectValue placeholder={prop.placeholder as string || "Select..."} />
              </SelectTrigger>
              <SelectContent>
                {options.map((option) => (
                  <SelectItem
        ...
        case 'expression':
          ...
          return (
            <Textarea
              ...
              value={value || ''}
              onChange={(e) => handleValueChange(prop.name, e.target.value)}
              placeholder={prop.placeholder as string || (inputType === 'code' ? 'Enter code...' : 'Enter expression...')}
              disabled={prop.readOnly}
